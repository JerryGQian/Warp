		...
		//In normal time
			...
			if ((gapSign > 0 && curr.toFloat() < settings.hExtend.h1 + gapSize) || (gapSign < 0 && curr.toFloat() < settings.hExtend.h1 - gapSize)) {
				...
				if (gapSign < 0) {
					...
					pd.elapsed = timeBetween(settings.hExtend.h1N, curr.toFloat());
				}
				else {
					pd.t = new Daytime(settings.hExtend.h1);
					pd.elapsed = timeBetween(settings.hExtend.h1, curr.toFloat());
				}
				return pd;
			}
			...
			if ((gapSign > 0 && curr.toFloat() < settings.hExtend.h2 + gapSize) || (gapSign < 0 && curr.toFloat() < settings.hExtend.h2 - gapSize)) {
				...
				if (gapSign < 0) {
					...
					pd.elapsed = timeBetween(settings.hExtend.h2N, curr.toFloat());
				}
				else {
					pd.t = new Daytime(settings.hExtend.h2);
					pd.elapsed = timeBetween(settings.hExtend.h2, curr.toFloat());
				}
				return pd;
			}
		}
